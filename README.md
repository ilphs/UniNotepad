...
- 다이어그램도 따로 확대·축소(`Ctrl/Cmd` + `=` `-` `0`)하고, 배경을 투명하게 두거나 원하는 색으로 지정할 수 있습니다
- 창을 키우면 다이어그램도 창 폭까지 함께 커집니다. 옆의 본문 글 폭은 `Preferences ▸ Preview ▸ Text width`에서 따로 정할 수 있어요(`0`을 넣으면 본문도 창 전체 폭)
...
| 하고 싶은 것 | 방법 |
|:--|:--|
| 미리보기 열기/닫기 | `Ctrl/Cmd + Shift + M` (또는 `View ▸ Toggle Preview`) — **탭마다 따로** 적용됩니다 |
| 창 하나만 보기 | 창 오른쪽 위 **×** 버튼(마우스를 올리면 나타납니다). 미리보기만 남기면 창 전체 폭을 씁니다 |
| 닫은 창 다시 열기 | 상태 표시줄의 `Editor` · `Preview` 누르기 (닫혀 있으면 `off`로 흐리게 표시됩니다) |
| 좌우 비율 조절 | 가운데 경계선을 마우스로 끌기 (**탭마다 따로** 기억, 다음에 켜도 유지) |
| 편집기·미리보기 확대 | `Ctrl/Cmd` + `=` `-` `0` — 편집기와 미리보기를 **각각 따로**, 확대율은 **탭마다** 유지 |
