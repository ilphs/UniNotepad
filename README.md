...
편집기를 스크롤하면 미리보기도 같은 위치로 따라 움직이고, 완성된 문서는
**HTML 파일로 내보내거나 인쇄(PDF로 저장)** 할 수 있습니다.

**줄 단위로 손봅니다.**
여러 줄을 골라 **가나다순 정렬, 중복·빈 줄 지우기, 뒤쪽 공백 정리,
대문자·소문자 바꾸기, 줄 위아래로 옮기기**를 한 번에 할 수 있습니다.
메뉴의 `Edit ▸ Line Operations`에 모여 있습니다.

**예전 한글 파일도 엽니다.**
요즘 표준인 UTF-8은 물론, 예전 윈도우 메모장에서 "ANSI"로 저장한
한글 파일(EUC-KR/CP949)도 알아서 판별해 제대로 엽니다. 혹시 잘못 읽히면
상태 표시줄에서 인코딩을 직접 골라 다시 읽어올 수 있습니다.
