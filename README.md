...
| **Bash** — 명령어 스크립트 | **HTML** — 웹 페이지 문서 |
| [![Bash 구문 강조](docs/syntax-bash.png)](https://ilphs.github.io/UniNotepad/#bash) | [![HTML 구문 강조](docs/syntax-html.png)](https://ilphs.github.io/UniNotepad/#html) |
...
**한글이 깨지지 않나요?**
요즘 표준인 UTF-8 파일은 (BOM이 있든 없든) 그대로 잘 열립니다. 줄바꿈 방식도
Windows식·Unix식을 알아서 알아본 뒤 저장할 때 원래대로 되돌려 놓습니다.

다만 **예전 윈도우 메모장에서 "ANSI"로 저장한 한글 파일(EUC-KR/CP949)은 아직
지원하지 않아 글자가 깨져 보입니다.** 지금 버전이 알아보는 인코딩은 UTF-8 계열뿐이고,
나머지는 임시 방식으로 열립니다. 오래된 한글 문서라면 다른 편집기에서 UTF-8로
바꿔 저장한 뒤 여시는 편이 안전합니다.
