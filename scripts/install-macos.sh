#!/usr/bin/env bash
#
# UniEdit — macOS 로컬 빌드 & 설치 스크립트
# 릴리스 빌드 후 /Applications 에 UniEdit.app 을 설치한다.
#
...
# 4) 산출물 확인
APP_SRC="src-tauri/target/release/bundle/macos/UniEdit.app"
if [ ! -d "$APP_SRC" ]; then
  echo "❌ 빌드 산출물을 찾지 못했습니다: $APP_SRC"; exit 1
fi

# 5) /Applications 에 설치
DEST="/Applications/UniEdit.app"
echo "==> 설치: $DEST"
rm -rf "$DEST"
cp -R "$APP_SRC" "$DEST"
...
echo "✅ 설치 완료"
echo "   앱:  $DEST"
echo "   실행: open \"$DEST\"   (또는 Launchpad/Spotlight 에서 'UniEdit')"
